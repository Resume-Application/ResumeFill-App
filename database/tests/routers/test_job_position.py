...
def test_create_job_position(client: TestClient, auth_headers):
    payload = {
        "name": "Test Company",
...
def test_get_job_position(client: TestClient, auth_headers):
    payload = {
        "name": "Test Company",
        "description": "A company for testing"
    }
    ...
    getResponse = client.get(f"/job_position/{jobResp.id}",
                             headers=auth_headers)
    assert getResponse.status_code == 200
    data = CompanyJobPositionResponse.model_validate(getResponse.json())
    assert data.title == "Software Engineer"

def test_get_job_position_not_exist(client: TestClient, auth_headers):
    getResponse = client.get(f"/job_position/500", headers=auth_headers)
